...
import com.example.paylasimmvvm.profile.ProfilFragmentDirections
import com.example.paylasimmvvm.profile.UserProfilFragmentDirections
            ...
            binding.tumLayout.setOnClickListener {
                if (!isProfileFragment){
                    if (gelenMudavim.mudavim_id!! != FirebaseAuth.getInstance().currentUser!!.uid){
                        val action =
                            UserProfilFragmentDirections.actionUserProfilFragmentSelf(gelenMudavim.mudavim_id!!)
                    ...
                    if (gelenMudavim.mudavim_id!! != FirebaseAuth.getInstance().currentUser!!.uid){
                        val action =
                            ProfilFragmentDirections.actionProfilFragmentToUserProfilFragment(
                                gelenMudavim.mudavim_id!!
                            )
                        Navigation.findNavController(itemView).navigate(action)
