package com.example.paylasimmvvm.comments
...
class comment_fragment : Fragment() , YorumlarRecyclerAdapter.OnReplyClickListener {
    ...
    private var tiklananUser=""
    private var isPost: Boolean = false
    private var postURL=""
    private var gonderiID=""
    private lateinit var mAuth: FirebaseAuth
    private lateinit var mUser: FirebaseUser
    private  var yorumYapilanGonderi: String? =null
    private var yorumKey:String?=null
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.twYorumPaylas.setOnClickListener {
            ...
            if(!TextUtils.isEmpty(yorum)){
                if (isPost){
                    val yeniYorumGonderiReference = FirebaseDatabase.getInstance().reference.child("kampanya").child(tiklananUser).child(
                        gonderiID
                    ).child("yorumlar").push()
    ...
    override fun onReplyClick(username: String, yorumID: String, yorum: String) {
        ...
        binding.twYorumPaylas.setOnClickListener {
            val yorumm=binding.etMesajEkleReply.text.toString().trim()
            if(!TextUtils.isEmpty(yorum)){

                if (isPost){
                    val yeniYorumGonderiReference = FirebaseDatabase.getInstance().reference.child("kampanya").child(tiklananUser).child(
                        gonderiID
                    ).child("yorumlar").child(yorumID).child("yanitlar").push()
                    val pushKey = yeniYorumGonderiReference.key
                    val yeniYorum = hashMapOf(
                        "yorum_key" to pushKey.toString(),
                        "user_id" to mUser.uid,
                    ...
                    Bildirimler.bildirimKaydet(
                        ...
                        gonderiID,
                        postURL,
                        binding.etMesajEkleReply.text.toString(),
                        pushKey,tiklananUser)}
                    yorumlarViewModel.gonderiYorumlariniAl(tiklananUser,gonderiID)
                }else{
                    val yeniYorumIsletmeReference = FirebaseDatabase.getInstance().reference.child("users").child("isletmeler").child(tiklananUser).child("yorumlar").child(yorumID).child("yanitlar").push()
                    val pushKey = yeniYorumIsletmeReference.key
                    val yeniYorum = hashMapOf(
                        "yorum_key" to pushKey.toString(),
                        "user_id" to mUser.uid,
                        "yorum" to binding.etMesajEkleReply.text.toString(),
                        "yorum_begeni" to "0",
                        "yorum_tarih" to ServerValue.TIMESTAMP)
                    yeniYorumIsletmeReference.setValue(yeniYorum)
                    if (tiklananUser!=FirebaseAuth.getInstance().currentUser!!.uid) {
                ...
                binding.etMesajEkleReply.setText("")
